...
export async function setupGceVm(dryRun: boolean): Promise<{ checks: Check[] }> {
  ...
  if (!dryRun) {
    ...
    const startupScript = [
      ...
      '',
      '# Set up mc-backup user (created by GCE guest agent via ssh-keys metadata)',
      '# Add to docker group + grant passwordless sudo for backup script',
      'if id mc-backup 2>/dev/null; then',
      '  usermod -aG docker mc-backup',
      '  echo "mc-backup ALL=(ALL) NOPASSWD: /mnt/disks/data/mc-backup.sh" > /etc/sudoers.d/mc-backup',
