...
export type CookieKey = '__session' | 'locale' | 'theme';
...
const saveStore = (cookies: Cookies, data: SessionStore, domain: string) => {
  ...
  if (stringifiedData.length > 3800) {
    console.warn('Cookie size is nearing the 4KB limit!', {
...
export const setCookie = (
  ...
  // console.debug('setCookie (in store)', { domain, key });
...
export const deleteCookie = (
  ...
  // console.log('deleteCookie (from store)', { domain, key });
  saveStore(cookies, store, domain);
};
